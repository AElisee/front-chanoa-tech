...
export default async function AdminProduitEditPage({
  ...
  try {
    const res = await apiClient.get<VariantDto[]>(`/produits/${id}/variants`, {
      headers,
    });
    variants = res.data ?? [];
    variants.sort(
        ...
        {variants.length > 0 && (
          <div className="mb-6 space-y-3">
            {variants.map((v) => {
              ...
              const label =
                Object.values(v.options).join(" / ") || "Sans option";
              return (
                <div
                  key={v.id}
                  className={`rounded-lg border p-4 ${v.is_active ? "" : "opacity-60"}`}
                >
                  ...
                  <form action={updateAction} className="space-y-3">
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      <div>
                        ...
                        <input
                          name="options"
                          defaultValue={JSON.stringify(v.options)}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div>
                ...
                <input
                  name="options"
                  required
                  placeholder='{"ram":"8 Go","stockage":"256 Go"}'
                  className="w-full rounded-md border bg-background px-2 py-1.5 font-mono text-xs"
                />
              </div>
              <div>
